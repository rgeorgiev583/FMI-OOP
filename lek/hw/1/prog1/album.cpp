...
size_t ni=0;
    ...
    for (LinkedListIterator<Song> i = songlist.begin(); i != songlist.end() && i + 1 != songlist.end(); i + 1 != songlist.end() ? i++ : i){size_t nj=ni+1;
        for (LinkedListIterator<Song> j = i + 1; j != songlist.end(); j != songlist.end() ? j++ : j){
            if (*i == *j)
            {
