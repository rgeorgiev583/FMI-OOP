...
Song::Song(): year(0), length(0), original(NULL)
{
    strcpy(title, "(Unknown title)");
    strcpy(artist, "(Unknown artist)");
}

Song::Song(const char* _title): year(0), length(0), original(NULL)
{
    setTitle(_title);
    strcpy(artist, "(Unknown artist)");
}

Song::Song(const char* _title, const char* _artist, int _year = 0, size_t _length = 0): year(_year), length(_length), original(NULL)
{
    setTitle(_title);
    setArtist(_artist);
}

Song::Song(const char* _title, const char* _artist, Song* _original, int _year = 0, size_t _length = 0): year(_year), length(_length), original(_original)
{
    setTitle(_title);
    setArtist(_artist);
}
...
void Song::print() const
{
    ...
    std::cout << "Original: " << (original ? original->title : "(Self)") << std::endl;
    std::cout << std::endl;
}
